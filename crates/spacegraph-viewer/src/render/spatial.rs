...
/// Socket reachability classification (`Exposure`, `exposure_bucket`) lives in the
/// headless core ([`spacegraph_graph::exposure`]); re-exported here so the existing
/// `crate::render::spatial::{Exposure, exposure_bucket}` call sites resolve and the
/// viewer keeps a single classification source (ADR-0001 / MP-v0.6.0 P2).
pub use spacegraph_graph::exposure::{exposure_bucket, Exposure};

/// Informational exposure tint (radial depth is the primary cue). Render-side —
/// returns a Bevy `Color`; the pure classification is in `spacegraph-graph`.
pub fn exposure_tint(e: Exposure) -> Color {
    match e {
...
#[cfg(test)]
mod perimeter_tests {
    use super::{aperture_style, ApertureStyle};
